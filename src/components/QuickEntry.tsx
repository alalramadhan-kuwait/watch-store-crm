...
export function QuickEntry() {
  ...
  useEffect(() => {
    if (settings && !staff) setStaff(settings.staffRoster[0] || '');
  }, [settings]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const newCase: Omit<Case, 'id'> = {
        caseId,
        dateLogged: format(now, 'yyyy-MM-dd'),
      ...
      await db.cases.add(newCase as Case);
      setLastStaff(staff);
      getProductSuggestions().then(setProducts);
  ...
  const locked = false; // Quick entry is always available even on locked days
  ...
  return (
    <div ref={formRef} className="px-4 pt-6 pb-32 max-w-lg mx-auto scroll-area">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* Lost Sale-specific: Lost Reason */}
        {caseType === 'Lost Sale' && (
          <div>
