...
// ── Customers: the list, the profile, and what may be changed ────────────────
// Everything here goes through customer_list() and customer_profile(), which
// run under the caller's own rules: a salesperson gets their customers, a
// manager the shop's, the owner everyone. The phone never filters.
...
/** The details that are ours to keep: name, dates, notes. Never the number — see updateCustomerPhone. */
...
/**
 * Change a customer's number. The database normalises it, refuses a number
 * that is not one, refuses one another customer already has, refuses to
 * change a number Lightspeed holds, and writes the audit row. Whatever it
 * says comes back as the error message, word for word.
 */
...
/** Managers and admins only; the database refuses anyone else. Null takes the assignment away. */
...
/**
 * Record that WhatsApp was opened for this customer with this template.
 * The database refuses it without a relationship, and on the shared phone it
 * is attributed to the salesperson named, not to the device.
 */
