...
- **2026-09-02** (later) — `Modal` (`components/ui.tsx`) now renders **full-screen on mobile** (a page-like sheet: no floating card / backdrop peek, sticky safe-area header with a back arrow, own scroll) and keeps the **centered dialog on desktop** (`sm:`). Applies to every record editor — so opening a PO (and any record) from a row or a notification tap reads as a page on phones instead of a cramped popup.
