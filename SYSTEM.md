...
| `meta-ads-sync` | false | cron + manual (admin/manager/marketing JWT, or `x-sync-key`) | Meta Graph v21.0 for the account in `meta_ads_config`, in four runs by `mode`: (none) campaigns + campaign totals/daily + the KD rate (open.er-api.com, unless pinned) + the purchase-tracking check; `detail` ad sets; `ads` ads and creatives; `ad_figures` ad totals/daily. Insights are asked of the **account** at each `level` and as **report jobs** (POST `/insights` → poll `report_run_id` → page), because three years with `action_values` is more than Meta answers inline ("unknown error, code 1"); pages are saved as they arrive (combined runs hit `WORKER_RESOURCE_LIMIT`). Totals go to `meta_insight_totals` (one row per level + object), daily to `meta_insight_daily`; campaign daily also still to `meta_ad_insights`. Fields include `frequency`, `action_values`, `purchase_roas`, `cost_per_action_type`. `mode:"diagnose"` reports token permissions, pixels, per-event match keys and sources, Meta's dataset quality and diagnostics. Values are stored as the strings Meta returned. |
| `daily-briefing` | false | cron (parked) | Email daily briefing (needs `RESEND_API_KEY`) |
| `instagram-connect` | true | Settings UI | Instagram token paste-in. **Unused** since 2026-09-26: `instagram-sync` now uses `META_ADS_TOKEN` |
| `instagram-sync` | false | cron + manual (admin/manager/marketing JWT or `instagram_auth.sync_key`) | **Instagram's own figures** for every business account on the token's pages (timekeeperkw, timegallerykw, timekeeperkwshop): yesterday's reach, profile views, accounts engaged, interactions and website taps onto yesterday's `instagram_daily` row, and the last 50 posts per account with reach, saves, shares, views, likes, comments, profile visits and follows into `instagram_media` (metrics requested in falling sets, since reels refuse feed-only metrics) |
| `meta-campaign-manage` | false (checks JWT itself) | Campaign Proposals page; `x-sync-key` for `check` only | **The only code that changes the ad account.** `check` (Meta `validate_only`, creates nothing), `approve` (owner: builds campaign + ad set + ad **PAUSED** from the proposal, using the Instagram post as the creative; a half-built campaign is deleted), `reject` (owner), `activate` (owner: switches on, end date counted from now), `pause` (owner/manager/marketing), `budget` (owner). Graph v23.0 (creative uses `instagram_user_id`). Campaign has no budget of its own (`is_adset_budget_sharing_enabled=false`); the ad set carries the approved KD/day converted to USD cents at the current rate. Every step to `ad_proposal_events` |
...
| `instagram-daily-sync` | `15 5 * * *` | 08:15 | `instagram-sync` (Instagram's own figures) |
...
| `meta-ads-daily-sync` | `30 5 * * *` | 08:30 | `meta-ads-sync` — campaigns, rate, tracking check |
| `meta-ads-detail-sync` | `40 5 * * *` | 08:40 | `meta-ads-sync` `mode:"detail"` — ad sets |
| `meta-ads-ad-sync` | `50 5 * * *` | 08:50 | `meta-ads-sync` `mode:"ads"` — ads and creatives |
| `meta-ads-ad-figures-sync` | `55 5 * * *` | 08:55 | `meta-ads-sync` `mode:"ad_figures"` |
| `growth-review-weekly` | `0 6 * * 0` | Sun 09:00 | `send_growth_review()` — the week's headline to owners and marketing |
...
- **2026-09-26** — **Meta and Instagram, measured properly; campaigns proposed, approved and built paused.** The audit of Media & Marketing found: Meta had credited no purchase to an ad since about 10 Aug while the website kept selling; the sync kept no purchase value, ROAS or cost per purchase, and nothing below the campaign; every sync added another copy of every campaign's totals (2,444 rows for 261 campaigns) and screens read one at random; the KD rate was hand-typed ten days earlier; Meta's days are Los Angeles days; the direct Instagram sync had never run. *Tracking:* the website is Shopify (time-keeper.com). Its pixel `992711141438349` receives Purchase with value, currency and order id (165 in the month to 26 Sep), and every sales ad set since May optimises for Purchase on it and every ad reports to it — the set-up is right. Meta's dataset-quality report gave the cause: Purchase match quality is 7.8/10 with email, phone and names, but **no purchase carries the ad-click ID (`fbc`)**, which 43% of add-to-carts and 58% of checkouts do — so a purchase cannot be tied to the ad that was tapped. About 2 in 3 purchases also arrive only from Shopify's server. The fix is in Shopify's Facebook & Instagram app (data sharing *Maximum*, cookie banner) and a test order through Events Manager, and needs the owner; a **tracking panel** on Meta Campaigns reports it daily. Meta's diagnostics also flag that product ids in pixel events do not match the catalog. *Sync:* `meta-ads-sync` rewritten (§9): report jobs, four runs, `meta_insight_totals` / `meta_insight_daily` / `meta_ad_sets` / `meta_ads`, money fields; screens read totals from `meta_insight_totals`. *Time and rate:* daily rate with source (Settings can pin one), and a panel stating Meta's zone and the hours behind Kuwait. *Instagram:* `instagram-sync` on the System User token (§9); Instagram page shows reach, visits, website taps and per-post reach/saves/shares/views. *Proposals:* `ad_proposals` + `ad_proposal_events`, page **Campaign Proposals**, function `meta-campaign-manage` (§9); owners approve, everything is built PAUSED, activation and budget are owner actions, pausing is anyone's. Tested with Meta's validate-only: campaign ✓ (after adding `is_adset_budget_sharing_enabled=false`); ad set not checkable (every recent sales campaign has a budget of its own, and a test ad set cannot sit under one) — it is proven on the first real approval, which deletes anything half-built if Meta refuses; creative refused because **the Meta app behind the token is in development mode** — the owner must switch it to Live. *Review:* `growth_review(p_end)` + page **Growth Review** + Sunday notification; Scale/Hold/Improve/Stop by stated rules; product names filled from stock (98 of 102 online lines had none). Migrations `…meta_figures_at_every_level_and_the_money`, `…instagram_figures_from_instagram_itself`, `…campaign_proposals_and_the_weekly_growth_review`, `…the_review_names_every_product_and_ads_get_their_own_run`. The 2,444 old lifetime rows in `meta_ad_insights` are no longer read and can be removed.
- **2026-09-24** — **Open pull requests reviewed.** #4 (18 Sep, never merged) carried two things main lacked, both now merged: the back office's Attendance page and Dashboard resolve a record to its person by `user_id` first and by the stored name copy only as a fallback, so a rename no longer detaches someone from their dated shift (lateness judged against the office 09:00 instead of their own start) or counts them twice in present-today; and the migrations folder, where `a_request_is_staged_whoever_inserts_it` was filed under 20260918084112 though the database recorded 20260918083226, `20260918102804_the_name_copy_follows_a_rename` was missing, and `20260918093608` differed from what was applied. Both folders now hold every version recorded since 18 Sep except the Watch Design Studio's five (20260920094846–20260921102836), which remain the known gap.
- **2026-09-24** — **A clock-in counts once.** Eman's clock-in went through twice, ten seconds apart (two open records, closed 14 s apart at 16:18), and every total added them: My Portal showed 12h 55m at about 3:55 pm for a day begun at 9:27, and `attendance_day_hours` held 13.69 h for a 6.85 h day. A new BEFORE INSERT trigger `attendance_one_open_shift` refuses a clock-in while the same person has an open shift younger than `attendance_abandon_hours()` (older open shifts were never clocked out and do not block), under a per-person advisory lock so simultaneous taps queue; both portals reload after a refusal so the page shows the shift that is already open. `dayHours()` (shared) and the `attendance_day_hours` view now count overlapping time once; across all 173 recorded days only that one day changed. My Portal (both apps) and the shop app's Home clock card now take today's total from `dayHours()` instead of their own sum. Migration `20260924140835`. The duplicate record was then removed with the owner's approval (`20260924142233`); it was the only pair of clock-ins less than five minutes apart on any account.
